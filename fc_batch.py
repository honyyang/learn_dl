...
class FullConnectedLayer(object):
    ...
    def forward(self, input_array):
        ...
        '''
        # 式2
        self.input = np.array(input_array)
        # 输出向量
        self.output = np.zeros((len(input_array), self.output_size, 1))
        for k in range(len(input_array)):
            self.output[k] = self.activator.forward(
                np.dot(self.W, input_array[k]) + self.b)
        return self.output
    ...
    def backward(self, delta_array, activator):
        ...
        # 式8
        # 前向误差
        self.delta = np.zeros(self.input.shape)
        # 权重导数
        self.W_grad = np.zeros(self.W.shape)
        # 偏置项导数
        self.b_grad = np.zeros(self.b.shape)
        for k in range(len(delta_array)):
            self.delta[k] = activator.backward(self.input[k]) * np.dot(
                self.W.T, delta_array[k])
            self.W_grad += np.dot(delta_array[k], self.input[k].T)
            self.b_grad += delta_array[k]
        self.W_grad /= len(delta_array)
        self.b_grad /= len(delta_array)
        return self.delta
...
class Network(object):
    ...
    def train(self, labels, data_set, rate, batch, epoch):
        '''
        训练函数
        labels: 样本标签
        data_set: 输入样本
...
def correct_ratio(network):
    ...
    correct = 0.0;
    for i in range(256):
        if normalizer.denorm(network.predict([normalizer.norm(i)]).reshape(8, 1)) == i:
            correct += 1.0
